 ...
 * - {@link LocalInstallStructuresCreateFragment}: to create or edit a(n) LocalInstallStructure,
 * - {@link LocalInstallStructuresDetailFragment}: to display the details of a(n) LocalInstallStructure,
 * - {@link LocalInstallStructuresListFragment}: to list LocalInstallStructures.
 *
 ...
 * what to do.
 */
public class LocalInstallStructuresActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<LocalInstallStructure> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link LocalInstallStructuresListFragment} is
     * getting instantiated (which will load {@link LocalInstallStructuresDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new LocalInstallStructuresListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable LocalInstallStructure entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        LocalInstallStructuresCreateFragment fragment = new LocalInstallStructuresCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable LocalInstallStructure entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new LocalInstallStructuresDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new LocalInstallStructuresDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable LocalInstallStructure entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link LocalInstallStructuresCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((LocalInstallStructuresActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            LocalInstallStructureViewModel viewModel = ViewModelProviders.of(getActivity()).get(LocalInstallStructureViewModel.class);
