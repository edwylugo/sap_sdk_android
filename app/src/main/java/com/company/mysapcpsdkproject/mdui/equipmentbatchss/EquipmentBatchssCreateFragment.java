...
import com.company.mysapcpsdkproject.databinding.FragmentEquipmentbatchssCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing EquipmentBatchs entity.
 * This fragment is contained in the {@link EquipmentBatchssActivity}.
 */
public class EquipmentBatchssCreateFragment extends InterfacedFragment<EquipmentBatchs> {

    private static final Logger LOGGER = LoggerFactory.getLogger(EquipmentBatchssCreateFragment.class);
    ...
    /** EquipmentBatchs object and it's copy: the modifications are done on the copied object. */
    private EquipmentBatchs equipmentBatchsEntity;
    private EquipmentBatchs equipmentBatchsEntityCopy;

    /** DataBinding generated class */
    private FragmentEquipmentbatchssCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** EquipmentBatchs ViewModel */
    ...
    /**
     * This fragment is used for both update and create for EquipmentBatchss to enter values for the properties.
     ...
     *            EquipmentBatchs if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.equipmentBatchs.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.equipmentBatchs.getLocalName();
        }

        ((EquipmentBatchssActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            equipmentBatchsEntity = createEquipmentBatchs();
        } else {
            equipmentBatchsEntity = viewModel.getSelectedEntity().getValue();
        }

        EquipmentBatchs workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (EquipmentBatchs)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            equipmentBatchsEntityCopy = (EquipmentBatchs) equipmentBatchsEntity.copy();
            equipmentBatchsEntityCopy.setEntityTag(equipmentBatchsEntity.getEntityTag());
            equipmentBatchsEntityCopy.setOldEntity(equipmentBatchsEntity);
            equipmentBatchsEntityCopy.setEditLink((equipmentBatchsEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            equipmentBatchsEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, equipmentBatchsEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isEquipmentBatchsValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((EquipmentBatchssActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(equipmentBatchsEntityCopy);
        } else {
            viewModel.update(equipmentBatchsEntityCopy);
        }
        return true;
    }

    /**
     * Create a new EquipmentBatchs instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new EquipmentBatchs instance
     */
    private EquipmentBatchs createEquipmentBatchs() {
        EquipmentBatchs equipmentBatchsEntity = new EquipmentBatchs(true);
        equipmentBatchsEntity.unsetDataValue(EquipmentBatchs.id);
        return equipmentBatchsEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<EquipmentBatchs> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(equipmentBatchsEntityCopy);
            }
            if( isMasterDetail ) {
                EquipmentBatchssListFragment listFragment = (EquipmentBatchssListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentEquipmentbatchssCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setEquipmentBatchs(equipmentBatchsEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isEquipmentBatchsValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_equipmentbatchs);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.equipmentBatchs.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<EquipmentBatchs> result) {
