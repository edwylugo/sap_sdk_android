...
import com.company.mysapcpsdkproject.databinding.FragmentCauseofconfirmationsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing CauseOfConfirmation entity.
 * This fragment is contained in the {@link CauseOfConfirmationsActivity}.
 */
public class CauseOfConfirmationsCreateFragment extends InterfacedFragment<CauseOfConfirmation> {

    private static final Logger LOGGER = LoggerFactory.getLogger(CauseOfConfirmationsCreateFragment.class);
    ...
    /** CauseOfConfirmation object and it's copy: the modifications are done on the copied object. */
    private CauseOfConfirmation causeOfConfirmationEntity;
    private CauseOfConfirmation causeOfConfirmationEntityCopy;

    /** DataBinding generated class */
    private FragmentCauseofconfirmationsCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** CauseOfConfirmation ViewModel */
    ...
    /**
     * This fragment is used for both update and create for CauseOfConfirmations to enter values for the properties.
     ...
     *            CauseOfConfirmation if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.causeOfConfirmation.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.causeOfConfirmation.getLocalName();
        }

        ((CauseOfConfirmationsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            causeOfConfirmationEntity = createCauseOfConfirmation();
        } else {
            causeOfConfirmationEntity = viewModel.getSelectedEntity().getValue();
        }

        CauseOfConfirmation workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (CauseOfConfirmation)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            causeOfConfirmationEntityCopy = (CauseOfConfirmation) causeOfConfirmationEntity.copy();
            causeOfConfirmationEntityCopy.setEntityTag(causeOfConfirmationEntity.getEntityTag());
            causeOfConfirmationEntityCopy.setOldEntity(causeOfConfirmationEntity);
            causeOfConfirmationEntityCopy.setEditLink((causeOfConfirmationEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            causeOfConfirmationEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, causeOfConfirmationEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isCauseOfConfirmationValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((CauseOfConfirmationsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(causeOfConfirmationEntityCopy);
        } else {
            viewModel.update(causeOfConfirmationEntityCopy);
        }
        return true;
    }

    /**
     * Create a new CauseOfConfirmation instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new CauseOfConfirmation instance
     */
    private CauseOfConfirmation createCauseOfConfirmation() {
        CauseOfConfirmation causeOfConfirmationEntity = new CauseOfConfirmation(true);
        causeOfConfirmationEntity.unsetDataValue(CauseOfConfirmation.plant);
        causeOfConfirmationEntity.unsetDataValue(CauseOfConfirmation.reasonForVariance);
        return causeOfConfirmationEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<CauseOfConfirmation> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(causeOfConfirmationEntityCopy);
            }
            if( isMasterDetail ) {
                CauseOfConfirmationsListFragment listFragment = (CauseOfConfirmationsListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentCauseofconfirmationsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setCauseOfConfirmation(causeOfConfirmationEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isCauseOfConfirmationValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_causeofconfirmation);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.causeOfConfirmation.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<CauseOfConfirmation> result) {
