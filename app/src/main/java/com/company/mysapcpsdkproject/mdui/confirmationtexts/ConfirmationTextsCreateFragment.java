...
import com.company.mysapcpsdkproject.databinding.FragmentConfirmationtextsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing ConfirmationText entity.
 * This fragment is contained in the {@link ConfirmationTextsActivity}.
 */
public class ConfirmationTextsCreateFragment extends InterfacedFragment<ConfirmationText> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ConfirmationTextsCreateFragment.class);
    ...
    /** ConfirmationText object and it's copy: the modifications are done on the copied object. */
    private ConfirmationText confirmationTextEntity;
    private ConfirmationText confirmationTextEntityCopy;

    /** DataBinding generated class */
    private FragmentConfirmationtextsCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** ConfirmationText ViewModel */
    ...
    /**
     * This fragment is used for both update and create for ConfirmationTexts to enter values for the properties.
     ...
     *            ConfirmationText if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.confirmationText.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.confirmationText.getLocalName();
        }

        ((ConfirmationTextsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            confirmationTextEntity = createConfirmationText();
        } else {
            confirmationTextEntity = viewModel.getSelectedEntity().getValue();
        }

        ConfirmationText workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (ConfirmationText)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            confirmationTextEntityCopy = (ConfirmationText) confirmationTextEntity.copy();
            confirmationTextEntityCopy.setEntityTag(confirmationTextEntity.getEntityTag());
            confirmationTextEntityCopy.setOldEntity(confirmationTextEntity);
            confirmationTextEntityCopy.setEditLink((confirmationTextEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            confirmationTextEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, confirmationTextEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isConfirmationTextValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((ConfirmationTextsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(confirmationTextEntityCopy);
        } else {
            viewModel.update(confirmationTextEntityCopy);
        }
        return true;
    }

    /**
     * Create a new ConfirmationText instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new ConfirmationText instance
     */
    private ConfirmationText createConfirmationText() {
        ConfirmationText confirmationTextEntity = new ConfirmationText(true);
        confirmationTextEntity.unsetDataValue(ConfirmationText.causeOfDeviation);
        return confirmationTextEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<ConfirmationText> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(confirmationTextEntityCopy);
            }
            if( isMasterDetail ) {
                ConfirmationTextsListFragment listFragment = (ConfirmationTextsListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentConfirmationtextsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setConfirmationText(confirmationTextEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isConfirmationTextValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_confirmationtext);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.confirmationText.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<ConfirmationText> result) {
