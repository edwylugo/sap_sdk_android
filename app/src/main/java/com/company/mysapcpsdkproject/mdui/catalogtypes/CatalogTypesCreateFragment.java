...
import com.company.mysapcpsdkproject.databinding.FragmentCatalogtypesCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing CatalogType entity.
 * This fragment is contained in the {@link CatalogTypesActivity}.
 */
public class CatalogTypesCreateFragment extends InterfacedFragment<CatalogType> {

    private static final Logger LOGGER = LoggerFactory.getLogger(CatalogTypesCreateFragment.class);
    ...
    /** CatalogType object and it's copy: the modifications are done on the copied object. */
    private CatalogType catalogTypeEntity;
    private CatalogType catalogTypeEntityCopy;

    /** DataBinding generated class */
    private FragmentCatalogtypesCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** CatalogType ViewModel */
    ...
    /**
     * This fragment is used for both update and create for CatalogTypes to enter values for the properties.
     ...
     *            CatalogType if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.catalogType.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.catalogType.getLocalName();
        }

        ((CatalogTypesActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            catalogTypeEntity = createCatalogType();
        } else {
            catalogTypeEntity = viewModel.getSelectedEntity().getValue();
        }

        CatalogType workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (CatalogType)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            catalogTypeEntityCopy = (CatalogType) catalogTypeEntity.copy();
            catalogTypeEntityCopy.setEntityTag(catalogTypeEntity.getEntityTag());
            catalogTypeEntityCopy.setOldEntity(catalogTypeEntity);
            catalogTypeEntityCopy.setEditLink((catalogTypeEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            catalogTypeEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, catalogTypeEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isCatalogTypeValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((CatalogTypesActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(catalogTypeEntityCopy);
        } else {
            viewModel.update(catalogTypeEntityCopy);
        }
        return true;
    }

    /**
     * Create a new CatalogType instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new CatalogType instance
     */
    private CatalogType createCatalogType() {
        CatalogType catalogTypeEntity = new CatalogType(true);
        catalogTypeEntity.unsetDataValue(CatalogType.catalog);
        catalogTypeEntity.unsetDataValue(CatalogType.catalogProfile);
        catalogTypeEntity.unsetDataValue(CatalogType.codeGroup);
        return catalogTypeEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<CatalogType> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(catalogTypeEntityCopy);
            }
            if( isMasterDetail ) {
                CatalogTypesListFragment listFragment = (CatalogTypesListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentCatalogtypesCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setCatalogType(catalogTypeEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isCatalogTypeValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_catalogtype);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.catalogType.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<CatalogType> result) {
