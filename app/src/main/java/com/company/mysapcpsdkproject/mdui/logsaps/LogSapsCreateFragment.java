...
import com.company.mysapcpsdkproject.databinding.FragmentLogsapsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing LogSap entity.
 * This fragment is contained in the {@link LogSapsActivity}.
 */
public class LogSapsCreateFragment extends InterfacedFragment<LogSap> {

    private static final Logger LOGGER = LoggerFactory.getLogger(LogSapsCreateFragment.class);
    ...
    /** LogSap object and it's copy: the modifications are done on the copied object. */
    private LogSap logSapEntity;
    private LogSap logSapEntityCopy;

    /** DataBinding generated class */
    private FragmentLogsapsCreateBinding binding;
    ...
    /**
     * This fragment is used for both update and create for LogSaps to enter values for the properties.
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.logSap.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.logSap.getLocalName();
        }

        ((LogSapsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            logSapEntity = createLogSap();
        } else {
            logSapEntity = viewModel.getSelectedEntity().getValue();
        }

        LogSap workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (LogSap)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            logSapEntityCopy = (LogSap) logSapEntity.copy();
            logSapEntityCopy.setEntityTag(logSapEntity.getEntityTag());
            logSapEntityCopy.setOldEntity(logSapEntity);
            logSapEntityCopy.setEditLink((logSapEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            logSapEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, logSapEntityCopy);
    ...
    private boolean onSaveItem() {
        ...
        ((LogSapsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(logSapEntityCopy);
        } else {
            viewModel.update(logSapEntityCopy);
        }
        return true;
    }

    /**
     * Create a new LogSap instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new LogSap instance
     */
    private LogSap createLogSap() {
        LogSap logSapEntity = new LogSap(true);
        logSapEntity.unsetDataValue(LogSap.idEvents);
        return logSapEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<LogSap> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(logSapEntityCopy);
            }
            if( isMasterDetail ) {
                LogSapsListFragment listFragment = (LogSapsListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentLogsapsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setLogSap(logSapEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isLogSapValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_logsap);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.logSap.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<LogSap> result) {
