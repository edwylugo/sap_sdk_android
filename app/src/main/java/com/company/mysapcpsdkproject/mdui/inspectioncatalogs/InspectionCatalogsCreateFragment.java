...
import com.company.mysapcpsdkproject.databinding.FragmentInspectioncatalogsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing InspectionCatalog entity.
 * This fragment is contained in the {@link InspectionCatalogsActivity}.
 */
public class InspectionCatalogsCreateFragment extends InterfacedFragment<InspectionCatalog> {

    private static final Logger LOGGER = LoggerFactory.getLogger(InspectionCatalogsCreateFragment.class);
    ...
    /** InspectionCatalog object and it's copy: the modifications are done on the copied object. */
    private InspectionCatalog inspectionCatalogEntity;
    private InspectionCatalog inspectionCatalogEntityCopy;

    /** DataBinding generated class */
    private FragmentInspectioncatalogsCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** InspectionCatalog ViewModel */
    ...
    /**
     * This fragment is used for both update and create for InspectionCatalogs to enter values for the properties.
     ...
     *            InspectionCatalog if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.inspectionCatalog.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.inspectionCatalog.getLocalName();
        }

        ((InspectionCatalogsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            inspectionCatalogEntity = createInspectionCatalog();
        } else {
            inspectionCatalogEntity = viewModel.getSelectedEntity().getValue();
        }

        InspectionCatalog workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (InspectionCatalog)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            inspectionCatalogEntityCopy = (InspectionCatalog) inspectionCatalogEntity.copy();
            inspectionCatalogEntityCopy.setEntityTag(inspectionCatalogEntity.getEntityTag());
            inspectionCatalogEntityCopy.setOldEntity(inspectionCatalogEntity);
            inspectionCatalogEntityCopy.setEditLink((inspectionCatalogEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            inspectionCatalogEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, inspectionCatalogEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isInspectionCatalogValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((InspectionCatalogsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(inspectionCatalogEntityCopy);
        } else {
            viewModel.update(inspectionCatalogEntityCopy);
        }
        return true;
    }

    /**
     * Create a new InspectionCatalog instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new InspectionCatalog instance
     */
    private InspectionCatalog createInspectionCatalog() {
        InspectionCatalog inspectionCatalogEntity = new InspectionCatalog(true);
        inspectionCatalogEntity.unsetDataValue(InspectionCatalog.catalog);
        return inspectionCatalogEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<InspectionCatalog> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(inspectionCatalogEntityCopy);
            }
            if( isMasterDetail ) {
                InspectionCatalogsListFragment listFragment = (InspectionCatalogsListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentInspectioncatalogsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setInspectionCatalog(inspectionCatalogEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isInspectionCatalogValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_inspectioncatalog);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.inspectionCatalog.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<InspectionCatalog> result) {
