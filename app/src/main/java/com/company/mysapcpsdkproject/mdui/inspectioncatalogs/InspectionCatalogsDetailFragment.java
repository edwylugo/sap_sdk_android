...
import com.company.mysapcpsdkproject.databinding.FragmentInspectioncatalogsDetailBinding;
...
import com.sap.cloud.mobile.odata.EntityKey;
/**
 * A fragment representing a single InspectionCatalog detail screen.
 * This fragment is contained in an InspectionCatalogsActivity.
 */
public class InspectionCatalogsDetailFragment extends InterfacedFragment<InspectionCatalog> {

    /** Generated data binding class based on layout file */
    private FragmentInspectioncatalogsDetailBinding binding;

    /** InspectionCatalog entity to be displayed */
    private InspectionCatalog inspectionCatalogEntity = null;
    ...
    /** Arguments: InspectionCatalog for display */
    @Override
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        viewModel.getSelectedEntity().observe(this, entity -> {
            inspectionCatalogEntity = entity;
            binding.setInspectionCatalog(entity);
            setupObjectHeader();
        });
    }
    ...
    private void onDeleteComplete(@NonNull OperationResult<InspectionCatalog> result) {
        ...
        listener.onFragmentStateChange(UIConstants.EVENT_DELETION_COMPLETED, inspectionCatalogEntity);
    }
     ...
     */
    private void setDetailImage(@NonNull ObjectHeader objectHeader, @NonNull InspectionCatalog inspectionCatalogEntity) {
        viewModel.downloadMedia(inspectionCatalogEntity, media -> {
            ...
            if (inspectionCatalogEntity.getDataValue(InspectionCatalog.codeGroup) != null && !inspectionCatalogEntity.getDataValue(InspectionCatalog.codeGroup).toString().isEmpty()) {
                objectHeader.setDetailImageCharacter(inspectionCatalogEntity.getDataValue(InspectionCatalog.codeGroup).toString().substring(0, 1));
            } else {
    ...
    /**
     * Setup ObjectHeader with an instance of InspectionCatalog
     */
    private void setupObjectHeader() {
        Toolbar secondToolbar = currentActivity.findViewById(R.id.secondaryToolbar);
        if (secondToolbar != null) {
            secondToolbar.setTitle(inspectionCatalogEntity.getEntityType().getLocalName());
        } else {
            currentActivity.setTitle(inspectionCatalogEntity.getEntityType().getLocalName());
        }
        ...
        if (objectHeader != null) {
            ...
            objectHeader.setSubheadline(EntityKeyUtil.getOptionalEntityKey(inspectionCatalogEntity));
            ...
            setDetailImage(objectHeader, inspectionCatalogEntity);
    ...
    private View setupDataBinding(LayoutInflater inflater, ViewGroup container) {
        binding = FragmentInspectioncatalogsDetailBinding.inflate(inflater, container, false);
