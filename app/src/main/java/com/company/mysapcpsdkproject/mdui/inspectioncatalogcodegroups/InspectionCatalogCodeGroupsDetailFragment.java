...
import com.company.mysapcpsdkproject.databinding.FragmentInspectioncatalogcodegroupsDetailBinding;
...
import com.sap.cloud.mobile.odata.EntityKey;
/**
 * A fragment representing a single InspectionCatalogCodeGroup detail screen.
 * This fragment is contained in an InspectionCatalogCodeGroupsActivity.
 */
public class InspectionCatalogCodeGroupsDetailFragment extends InterfacedFragment<InspectionCatalogCodeGroup> {

    /** Generated data binding class based on layout file */
    private FragmentInspectioncatalogcodegroupsDetailBinding binding;

    /** InspectionCatalogCodeGroup entity to be displayed */
    private InspectionCatalogCodeGroup inspectionCatalogCodeGroupEntity = null;
    ...
    /** Arguments: InspectionCatalogCodeGroup for display */
    @Override
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        viewModel.getSelectedEntity().observe(this, entity -> {
            inspectionCatalogCodeGroupEntity = entity;
            binding.setInspectionCatalogCodeGroup(entity);
            setupObjectHeader();
        });
    }
    ...
    private void onDeleteComplete(@NonNull OperationResult<InspectionCatalogCodeGroup> result) {
        ...
        listener.onFragmentStateChange(UIConstants.EVENT_DELETION_COMPLETED, inspectionCatalogCodeGroupEntity);
    }
     ...
     */
    private void setDetailImage(@NonNull ObjectHeader objectHeader, @NonNull InspectionCatalogCodeGroup inspectionCatalogCodeGroupEntity) {
        viewModel.downloadMedia(inspectionCatalogCodeGroupEntity, media -> {
            ...
            if (inspectionCatalogCodeGroupEntity.getDataValue(InspectionCatalogCodeGroup.dataRecordUsed) != null && !inspectionCatalogCodeGroupEntity.getDataValue(InspectionCatalogCodeGroup.dataRecordUsed).toString().isEmpty()) {
                objectHeader.setDetailImageCharacter(inspectionCatalogCodeGroupEntity.getDataValue(InspectionCatalogCodeGroup.dataRecordUsed).toString().substring(0, 1));
            } else {
    ...
    /**
     * Setup ObjectHeader with an instance of InspectionCatalogCodeGroup
     */
    private void setupObjectHeader() {
        Toolbar secondToolbar = currentActivity.findViewById(R.id.secondaryToolbar);
        if (secondToolbar != null) {
            secondToolbar.setTitle(inspectionCatalogCodeGroupEntity.getEntityType().getLocalName());
        } else {
            currentActivity.setTitle(inspectionCatalogCodeGroupEntity.getEntityType().getLocalName());
        }
        ...
        if (objectHeader != null) {
            ...
            objectHeader.setSubheadline(EntityKeyUtil.getOptionalEntityKey(inspectionCatalogCodeGroupEntity));
            ...
            setDetailImage(objectHeader, inspectionCatalogCodeGroupEntity);
    ...
    private View setupDataBinding(LayoutInflater inflater, ViewGroup container) {
        binding = FragmentInspectioncatalogcodegroupsDetailBinding.inflate(inflater, container, false);
