...
import com.company.mysapcpsdkproject.databinding.FragmentOrdertypetextsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing OrderTypeText entity.
 * This fragment is contained in the {@link OrderTypeTextsActivity}.
 */
public class OrderTypeTextsCreateFragment extends InterfacedFragment<OrderTypeText> {

    private static final Logger LOGGER = LoggerFactory.getLogger(OrderTypeTextsCreateFragment.class);
    ...
    /** OrderTypeText object and it's copy: the modifications are done on the copied object. */
    private OrderTypeText orderTypeTextEntity;
    private OrderTypeText orderTypeTextEntityCopy;

    /** DataBinding generated class */
    private FragmentOrdertypetextsCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** OrderTypeText ViewModel */
    ...
    /**
     * This fragment is used for both update and create for OrderTypeTexts to enter values for the properties.
     ...
     *            OrderTypeText if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.orderTypeText.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.orderTypeText.getLocalName();
        }

        ((OrderTypeTextsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            orderTypeTextEntity = createOrderTypeText();
        } else {
            orderTypeTextEntity = viewModel.getSelectedEntity().getValue();
        }

        OrderTypeText workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (OrderTypeText)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            orderTypeTextEntityCopy = (OrderTypeText) orderTypeTextEntity.copy();
            orderTypeTextEntityCopy.setEntityTag(orderTypeTextEntity.getEntityTag());
            orderTypeTextEntityCopy.setOldEntity(orderTypeTextEntity);
            orderTypeTextEntityCopy.setEditLink((orderTypeTextEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            orderTypeTextEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, orderTypeTextEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isOrderTypeTextValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((OrderTypeTextsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(orderTypeTextEntityCopy);
        } else {
            viewModel.update(orderTypeTextEntityCopy);
        }
        return true;
    }

    /**
     * Create a new OrderTypeText instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new OrderTypeText instance
     */
    private OrderTypeText createOrderTypeText() {
        OrderTypeText orderTypeTextEntity = new OrderTypeText(true);
        orderTypeTextEntity.unsetDataValue(OrderTypeText.salesDocumentType);
        return orderTypeTextEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<OrderTypeText> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(orderTypeTextEntityCopy);
            }
            if( isMasterDetail ) {
                OrderTypeTextsListFragment listFragment = (OrderTypeTextsListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentOrdertypetextsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setOrderTypeText(orderTypeTextEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isOrderTypeTextValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_ordertypetext);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.orderTypeText.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<OrderTypeText> result) {
