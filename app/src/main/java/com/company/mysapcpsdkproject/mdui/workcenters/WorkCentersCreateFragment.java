...
import com.company.mysapcpsdkproject.databinding.FragmentWorkcentersCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing WorkCenter entity.
 * This fragment is contained in the {@link WorkCentersActivity}.
 */
public class WorkCentersCreateFragment extends InterfacedFragment<WorkCenter> {

    private static final Logger LOGGER = LoggerFactory.getLogger(WorkCentersCreateFragment.class);
    ...
    /** WorkCenter object and it's copy: the modifications are done on the copied object. */
    private WorkCenter workCenterEntity;
    private WorkCenter workCenterEntityCopy;

    /** DataBinding generated class */
    private FragmentWorkcentersCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** WorkCenter ViewModel */
    ...
    /**
     * This fragment is used for both update and create for WorkCenters to enter values for the properties.
     ...
     *            WorkCenter if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.workCenter.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.workCenter.getLocalName();
        }

        ((WorkCentersActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            workCenterEntity = createWorkCenter();
        } else {
            workCenterEntity = viewModel.getSelectedEntity().getValue();
        }

        WorkCenter workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (WorkCenter)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            workCenterEntityCopy = (WorkCenter) workCenterEntity.copy();
            workCenterEntityCopy.setEntityTag(workCenterEntity.getEntityTag());
            workCenterEntityCopy.setOldEntity(workCenterEntity);
            workCenterEntityCopy.setEditLink((workCenterEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            workCenterEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, workCenterEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isWorkCenterValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((WorkCentersActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(workCenterEntityCopy);
        } else {
            viewModel.update(workCenterEntityCopy);
        }
        return true;
    }

    /**
     * Create a new WorkCenter instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new WorkCenter instance
     */
    private WorkCenter createWorkCenter() {
        WorkCenter workCenterEntity = new WorkCenter(true);
        workCenterEntity.unsetDataValue(WorkCenter.plant);
        workCenterEntity.unsetDataValue(WorkCenter.workCenter);
        return workCenterEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<WorkCenter> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(workCenterEntityCopy);
            }
            if( isMasterDetail ) {
                WorkCentersListFragment listFragment = (WorkCentersListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentWorkcentersCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setWorkCenter(workCenterEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isWorkCenterValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_workcenter);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.workCenter.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<WorkCenter> result) {
