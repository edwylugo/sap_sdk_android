...
import com.company.mysapcpsdkproject.databinding.FragmentEnrollordertypesCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing EnrollOrderType entity.
 * This fragment is contained in the {@link EnrollOrderTypesActivity}.
 */
public class EnrollOrderTypesCreateFragment extends InterfacedFragment<EnrollOrderType> {

    private static final Logger LOGGER = LoggerFactory.getLogger(EnrollOrderTypesCreateFragment.class);
    ...
    /** EnrollOrderType object and it's copy: the modifications are done on the copied object. */
    private EnrollOrderType enrollOrderTypeEntity;
    private EnrollOrderType enrollOrderTypeEntityCopy;

    /** DataBinding generated class */
    private FragmentEnrollordertypesCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** EnrollOrderType ViewModel */
    ...
    /**
     * This fragment is used for both update and create for EnrollOrderTypes to enter values for the properties.
     ...
     *            EnrollOrderType if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.enrollOrderType.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.enrollOrderType.getLocalName();
        }

        ((EnrollOrderTypesActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            enrollOrderTypeEntity = createEnrollOrderType();
        } else {
            enrollOrderTypeEntity = viewModel.getSelectedEntity().getValue();
        }

        EnrollOrderType workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (EnrollOrderType)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            enrollOrderTypeEntityCopy = (EnrollOrderType) enrollOrderTypeEntity.copy();
            enrollOrderTypeEntityCopy.setEntityTag(enrollOrderTypeEntity.getEntityTag());
            enrollOrderTypeEntityCopy.setOldEntity(enrollOrderTypeEntity);
            enrollOrderTypeEntityCopy.setEditLink((enrollOrderTypeEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            enrollOrderTypeEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, enrollOrderTypeEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isEnrollOrderTypeValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((EnrollOrderTypesActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(enrollOrderTypeEntityCopy);
        } else {
            viewModel.update(enrollOrderTypeEntityCopy);
        }
        return true;
    }

    /**
     * Create a new EnrollOrderType instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new EnrollOrderType instance
     */
    private EnrollOrderType createEnrollOrderType() {
        EnrollOrderType enrollOrderTypeEntity = new EnrollOrderType(true);
        enrollOrderTypeEntity.unsetDataValue(EnrollOrderType.id);
        enrollOrderTypeEntity.unsetDataValue(EnrollOrderType.personnelNumber);
        enrollOrderTypeEntity.unsetDataValue(EnrollOrderType.suser);
        return enrollOrderTypeEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<EnrollOrderType> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(enrollOrderTypeEntityCopy);
            }
            if( isMasterDetail ) {
                EnrollOrderTypesListFragment listFragment = (EnrollOrderTypesListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentEnrollordertypesCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setEnrollOrderType(enrollOrderTypeEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isEnrollOrderTypeValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_enrollordertype);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.enrollOrderType.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<EnrollOrderType> result) {
