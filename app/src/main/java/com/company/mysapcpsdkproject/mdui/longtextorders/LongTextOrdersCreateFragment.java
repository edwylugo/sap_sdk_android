...
import com.company.mysapcpsdkproject.databinding.FragmentLongtextordersCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing LongTextOrder entity.
 * This fragment is contained in the {@link LongTextOrdersActivity}.
 */
public class LongTextOrdersCreateFragment extends InterfacedFragment<LongTextOrder> {

    private static final Logger LOGGER = LoggerFactory.getLogger(LongTextOrdersCreateFragment.class);
    ...
    /** LongTextOrder object and it's copy: the modifications are done on the copied object. */
    private LongTextOrder longTextOrderEntity;
    private LongTextOrder longTextOrderEntityCopy;

    /** DataBinding generated class */
    private FragmentLongtextordersCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** LongTextOrder ViewModel */
    ...
    /**
     * This fragment is used for both update and create for LongTextOrders to enter values for the properties.
     ...
     *            LongTextOrder if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.longTextOrder.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.longTextOrder.getLocalName();
        }

        ((LongTextOrdersActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            longTextOrderEntity = createLongTextOrder();
        } else {
            longTextOrderEntity = viewModel.getSelectedEntity().getValue();
        }

        LongTextOrder workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (LongTextOrder)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            longTextOrderEntityCopy = (LongTextOrder) longTextOrderEntity.copy();
            longTextOrderEntityCopy.setEntityTag(longTextOrderEntity.getEntityTag());
            longTextOrderEntityCopy.setOldEntity(longTextOrderEntity);
            longTextOrderEntityCopy.setEditLink((longTextOrderEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            longTextOrderEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, longTextOrderEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isLongTextOrderValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((LongTextOrdersActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(longTextOrderEntityCopy);
        } else {
            viewModel.update(longTextOrderEntityCopy);
        }
        return true;
    }

    /**
     * Create a new LongTextOrder instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new LongTextOrder instance
     */
    private LongTextOrder createLongTextOrder() {
        LongTextOrder longTextOrderEntity = new LongTextOrder(true);
        longTextOrderEntity.unsetDataValue(LongTextOrder.operationNumber);
        longTextOrderEntity.unsetDataValue(LongTextOrder.orderNumber);
        return longTextOrderEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<LongTextOrder> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(longTextOrderEntityCopy);
            }
            if( isMasterDetail ) {
                LongTextOrdersListFragment listFragment = (LongTextOrdersListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentLongtextordersCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setLongTextOrder(longTextOrderEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isLongTextOrderValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_longtextorder);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.longTextOrder.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<LongTextOrder> result) {
