 ...
 * - {@link MaintenanceNotificationsCreateFragment}: to create or edit a(n) MaintenanceNotification,
 * - {@link MaintenanceNotificationsDetailFragment}: to display the details of a(n) MaintenanceNotification,
 * - {@link MaintenanceNotificationsListFragment}: to list MaintenanceNotifications.
 *
 ...
 * what to do.
 */
public class MaintenanceNotificationsActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<MaintenanceNotification> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link MaintenanceNotificationsListFragment} is
     * getting instantiated (which will load {@link MaintenanceNotificationsDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new MaintenanceNotificationsListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable MaintenanceNotification entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        MaintenanceNotificationsCreateFragment fragment = new MaintenanceNotificationsCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable MaintenanceNotification entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new MaintenanceNotificationsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new MaintenanceNotificationsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable MaintenanceNotification entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link MaintenanceNotificationsCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((MaintenanceNotificationsActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            MaintenanceNotificationViewModel viewModel = ViewModelProviders.of(getActivity()).get(MaintenanceNotificationViewModel.class);
