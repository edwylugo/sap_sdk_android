...
import com.company.mysapcpsdkproject.databinding.FragmentEquipmentsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing Equipment entity.
 * This fragment is contained in the {@link EquipmentsActivity}.
 */
public class EquipmentsCreateFragment extends InterfacedFragment<Equipment> {

    private static final Logger LOGGER = LoggerFactory.getLogger(EquipmentsCreateFragment.class);
    ...
    /** Equipment object and it's copy: the modifications are done on the copied object. */
    private Equipment equipmentEntity;
    private Equipment equipmentEntityCopy;

    /** DataBinding generated class */
    private FragmentEquipmentsCreateBinding binding;
    ...
    /**
     * This fragment is used for both update and create for Equipments to enter values for the properties.
     ...
     *            Equipment if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.equipment.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.equipment.getLocalName();
        }

        ((EquipmentsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            equipmentEntity = createEquipment();
        } else {
            equipmentEntity = viewModel.getSelectedEntity().getValue();
        }

        Equipment workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (Equipment)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            equipmentEntityCopy = (Equipment) equipmentEntity.copy();
            equipmentEntityCopy.setEntityTag(equipmentEntity.getEntityTag());
            equipmentEntityCopy.setOldEntity(equipmentEntity);
            equipmentEntityCopy.setEditLink((equipmentEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            equipmentEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, equipmentEntityCopy);
    ...
    private boolean onSaveItem() {
        ...
        ((EquipmentsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(equipmentEntityCopy);
        } else {
            viewModel.update(equipmentEntityCopy);
        }
        return true;
    }

    /**
     * Create a new Equipment instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new Equipment instance
     */
    private Equipment createEquipment() {
        Equipment equipmentEntity = new Equipment(true);
        equipmentEntity.unsetDataValue(Equipment.numberEquipment);
        return equipmentEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<Equipment> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(equipmentEntityCopy);
            }
            if( isMasterDetail ) {
                EquipmentsListFragment listFragment = (EquipmentsListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentEquipmentsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setEquipment(equipmentEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isEquipmentValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_equipment);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.equipment.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<Equipment> result) {
