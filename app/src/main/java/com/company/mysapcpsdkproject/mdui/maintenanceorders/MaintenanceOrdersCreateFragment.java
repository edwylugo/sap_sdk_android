...
import com.company.mysapcpsdkproject.databinding.FragmentMaintenanceordersCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing MaintenanceOrder entity.
 * This fragment is contained in the {@link MaintenanceOrdersActivity}.
 */
public class MaintenanceOrdersCreateFragment extends InterfacedFragment<MaintenanceOrder> {

    private static final Logger LOGGER = LoggerFactory.getLogger(MaintenanceOrdersCreateFragment.class);
    ...
    /** MaintenanceOrder object and it's copy: the modifications are done on the copied object. */
    private MaintenanceOrder maintenanceOrderEntity;
    private MaintenanceOrder maintenanceOrderEntityCopy;

    /** DataBinding generated class */
    private FragmentMaintenanceordersCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** MaintenanceOrder ViewModel */
    ...
    /**
     * This fragment is used for both update and create for MaintenanceOrders to enter values for the properties.
     ...
     *            MaintenanceOrder if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.maintenanceOrder.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.maintenanceOrder.getLocalName();
        }

        ((MaintenanceOrdersActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            maintenanceOrderEntity = createMaintenanceOrder();
        } else {
            maintenanceOrderEntity = viewModel.getSelectedEntity().getValue();
        }

        MaintenanceOrder workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (MaintenanceOrder)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            maintenanceOrderEntityCopy = (MaintenanceOrder) maintenanceOrderEntity.copy();
            maintenanceOrderEntityCopy.setEntityTag(maintenanceOrderEntity.getEntityTag());
            maintenanceOrderEntityCopy.setOldEntity(maintenanceOrderEntity);
            maintenanceOrderEntityCopy.setEditLink((maintenanceOrderEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            maintenanceOrderEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, maintenanceOrderEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isMaintenanceOrderValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((MaintenanceOrdersActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(maintenanceOrderEntityCopy);
        } else {
            viewModel.update(maintenanceOrderEntityCopy);
        }
        return true;
    }

    /**
     * Create a new MaintenanceOrder instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new MaintenanceOrder instance
     */
    private MaintenanceOrder createMaintenanceOrder() {
        MaintenanceOrder maintenanceOrderEntity = new MaintenanceOrder(true);
        maintenanceOrderEntity.unsetDataValue(MaintenanceOrder.orderNumber);
        return maintenanceOrderEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<MaintenanceOrder> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(maintenanceOrderEntityCopy);
            }
            if( isMasterDetail ) {
                MaintenanceOrdersListFragment listFragment = (MaintenanceOrdersListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentMaintenanceordersCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setMaintenanceOrder(maintenanceOrderEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isMaintenanceOrderValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_maintenanceorder);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.maintenanceOrder.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<MaintenanceOrder> result) {
