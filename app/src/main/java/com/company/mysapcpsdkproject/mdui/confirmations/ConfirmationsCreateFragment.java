...
import com.company.mysapcpsdkproject.databinding.FragmentConfirmationsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing Confirmation entity.
 * This fragment is contained in the {@link ConfirmationsActivity}.
 */
public class ConfirmationsCreateFragment extends InterfacedFragment<Confirmation> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ConfirmationsCreateFragment.class);
    ...
    /** Confirmation object and it's copy: the modifications are done on the copied object. */
    private Confirmation confirmationEntity;
    private Confirmation confirmationEntityCopy;

    /** DataBinding generated class */
    private FragmentConfirmationsCreateBinding binding;
    ...
    /**
     * This fragment is used for both update and create for Confirmations to enter values for the properties.
     ...
     *            Confirmation if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.confirmation.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.confirmation.getLocalName();
        }

        ((ConfirmationsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            confirmationEntity = createConfirmation();
        } else {
            confirmationEntity = viewModel.getSelectedEntity().getValue();
        }

        Confirmation workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (Confirmation)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            confirmationEntityCopy = (Confirmation) confirmationEntity.copy();
            confirmationEntityCopy.setEntityTag(confirmationEntity.getEntityTag());
            confirmationEntityCopy.setOldEntity(confirmationEntity);
            confirmationEntityCopy.setEditLink((confirmationEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            confirmationEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, confirmationEntityCopy);
    ...
    private boolean onSaveItem() {
        ...
        ((ConfirmationsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(confirmationEntityCopy);
        } else {
            viewModel.update(confirmationEntityCopy);
        }
        return true;
    }

    /**
     * Create a new Confirmation instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new Confirmation instance
     */
    private Confirmation createConfirmation() {
        Confirmation confirmationEntity = new Confirmation(true);
        confirmationEntity.unsetDataValue(Confirmation.id);
        confirmationEntity.unsetDataValue(Confirmation.operationNumber);
        confirmationEntity.unsetDataValue(Confirmation.orderNumber);
        confirmationEntity.unsetDataValue(Confirmation.subOperation);
        return confirmationEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<Confirmation> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(confirmationEntityCopy);
            }
            if( isMasterDetail ) {
                ConfirmationsListFragment listFragment = (ConfirmationsListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentConfirmationsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setConfirmation(confirmationEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isConfirmationValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_confirmation);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.confirmation.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<Confirmation> result) {
