 ...
 * - {@link MaintenanceNotificationCausesCreateFragment}: to create or edit a(n) MaintenanceNotificationCause,
 * - {@link MaintenanceNotificationCausesDetailFragment}: to display the details of a(n) MaintenanceNotificationCause,
 * - {@link MaintenanceNotificationCausesListFragment}: to list MaintenanceNotificationCauses.
 *
 ...
 * what to do.
 */
public class MaintenanceNotificationCausesActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<MaintenanceNotificationCause> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link MaintenanceNotificationCausesListFragment} is
     * getting instantiated (which will load {@link MaintenanceNotificationCausesDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new MaintenanceNotificationCausesListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable MaintenanceNotificationCause entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        MaintenanceNotificationCausesCreateFragment fragment = new MaintenanceNotificationCausesCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable MaintenanceNotificationCause entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new MaintenanceNotificationCausesDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new MaintenanceNotificationCausesDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable MaintenanceNotificationCause entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link MaintenanceNotificationCausesCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((MaintenanceNotificationCausesActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            MaintenanceNotificationCauseViewModel viewModel = ViewModelProviders.of(getActivity()).get(MaintenanceNotificationCauseViewModel.class);
