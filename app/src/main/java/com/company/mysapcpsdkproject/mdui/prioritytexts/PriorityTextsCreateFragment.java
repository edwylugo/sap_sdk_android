...
import com.company.mysapcpsdkproject.databinding.FragmentPrioritytextsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing PriorityText entity.
 * This fragment is contained in the {@link PriorityTextsActivity}.
 */
public class PriorityTextsCreateFragment extends InterfacedFragment<PriorityText> {

    private static final Logger LOGGER = LoggerFactory.getLogger(PriorityTextsCreateFragment.class);
    ...
    /** PriorityText object and it's copy: the modifications are done on the copied object. */
    private PriorityText priorityTextEntity;
    private PriorityText priorityTextEntityCopy;

    /** DataBinding generated class */
    private FragmentPrioritytextsCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** PriorityText ViewModel */
    ...
    /**
     * This fragment is used for both update and create for PriorityTexts to enter values for the properties.
     ...
     *            PriorityText if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.priorityText.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.priorityText.getLocalName();
        }

        ((PriorityTextsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            priorityTextEntity = createPriorityText();
        } else {
            priorityTextEntity = viewModel.getSelectedEntity().getValue();
        }

        PriorityText workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (PriorityText)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            priorityTextEntityCopy = (PriorityText) priorityTextEntity.copy();
            priorityTextEntityCopy.setEntityTag(priorityTextEntity.getEntityTag());
            priorityTextEntityCopy.setOldEntity(priorityTextEntity);
            priorityTextEntityCopy.setEditLink((priorityTextEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            priorityTextEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, priorityTextEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isPriorityTextValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((PriorityTextsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(priorityTextEntityCopy);
        } else {
            viewModel.update(priorityTextEntityCopy);
        }
        return true;
    }

    /**
     * Create a new PriorityText instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new PriorityText instance
     */
    private PriorityText createPriorityText() {
        PriorityText priorityTextEntity = new PriorityText(true);
        priorityTextEntity.unsetDataValue(PriorityText.priority);
        priorityTextEntity.unsetDataValue(PriorityText.priorityType);
        return priorityTextEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<PriorityText> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(priorityTextEntityCopy);
            }
            if( isMasterDetail ) {
                PriorityTextsListFragment listFragment = (PriorityTextsListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentPrioritytextsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setPriorityText(priorityTextEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isPriorityTextValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_prioritytext);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.priorityText.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<PriorityText> result) {
