...
import com.company.mysapcpsdkproject.databinding.FragmentActioneventssCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing ActionEvents entity.
 * This fragment is contained in the {@link ActionEventssActivity}.
 */
public class ActionEventssCreateFragment extends InterfacedFragment<ActionEvents> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActionEventssCreateFragment.class);
    ...
    /** ActionEvents object and it's copy: the modifications are done on the copied object. */
    private ActionEvents actionEventsEntity;
    private ActionEvents actionEventsEntityCopy;

    /** DataBinding generated class */
    private FragmentActioneventssCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** ActionEvents ViewModel */
    ...
    /**
     * This fragment is used for both update and create for ActionEventss to enter values for the properties.
     ...
     *            ActionEvents if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.actionEvents.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.actionEvents.getLocalName();
        }

        ((ActionEventssActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            actionEventsEntity = createActionEvents();
        } else {
            actionEventsEntity = viewModel.getSelectedEntity().getValue();
        }

        ActionEvents workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (ActionEvents)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            actionEventsEntityCopy = (ActionEvents) actionEventsEntity.copy();
            actionEventsEntityCopy.setEntityTag(actionEventsEntity.getEntityTag());
            actionEventsEntityCopy.setOldEntity(actionEventsEntity);
            actionEventsEntityCopy.setEditLink((actionEventsEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            actionEventsEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, actionEventsEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isActionEventsValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((ActionEventssActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(actionEventsEntityCopy);
        } else {
            viewModel.update(actionEventsEntityCopy);
        }
        return true;
    }

    /**
     * Create a new ActionEvents instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new ActionEvents instance
     */
    private ActionEvents createActionEvents() {
        ActionEvents actionEventsEntity = new ActionEvents(true);
        actionEventsEntity.unsetDataValue(ActionEvents.idEvents);
        return actionEventsEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<ActionEvents> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(actionEventsEntityCopy);
            }
            if( isMasterDetail ) {
                ActionEventssListFragment listFragment = (ActionEventssListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentActioneventssCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setActionEvents(actionEventsEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isActionEventsValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_actionevents);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.actionEvents.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<ActionEvents> result) {
