...
import com.company.mysapcpsdkproject.databinding.FragmentUsersCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing User entity.
 * This fragment is contained in the {@link UsersActivity}.
 */
public class UsersCreateFragment extends InterfacedFragment<User> {

    private static final Logger LOGGER = LoggerFactory.getLogger(UsersCreateFragment.class);
    ...
    /** User object and it's copy: the modifications are done on the copied object. */
    private User userEntity;
    private User userEntityCopy;
    ...
    /**
     * This fragment is used for both update and create for Users to enter values for the properties.
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.user.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.user.getLocalName();
        }

        ((UsersActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            ...
            userEntity = viewModel.getSelectedEntity().getValue();
        }

        User workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (User)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            userEntityCopy = (User) userEntity.copy();
            userEntityCopy.setEntityTag(userEntity.getEntityTag());
            userEntityCopy.setOldEntity(userEntity);
            userEntityCopy.setEditLink((userEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            userEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, userEntityCopy);
    ...
    private boolean onSaveItem() {
        ...
        ((UsersActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(userEntityCopy);
        } else {
            viewModel.update(userEntityCopy);
        }
        return true;
    }

    /**
     * Create a new User instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new User instance
     */
    private User createUser() {
        User userEntity = new User(true);
        userEntity.unsetDataValue(User.personnelNumber);
        userEntity.unsetDataValue(User.suser);
        return userEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<User> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(userEntityCopy);
            }
            if( isMasterDetail ) {
                UsersListFragment listFragment = (UsersListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        ...
        binding.setUser(userEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isUserValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_user);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.user.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<User> result) {
