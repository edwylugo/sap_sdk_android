...
import com.company.mysapcpsdkproject.databinding.FragmentConfigdatabatchsDetailBinding;
...
/**
 * A fragment representing a single ConfigDataBatch detail screen.
 * This fragment is contained in an ConfigDataBatchsActivity.
 */
public class ConfigDataBatchsDetailFragment extends InterfacedFragment<ConfigDataBatch> {

    /** Generated data binding class based on layout file */
    private FragmentConfigdatabatchsDetailBinding binding;

    /** ConfigDataBatch entity to be displayed */
    private ConfigDataBatch configDataBatchEntity = null;
    ...
    /** Arguments: ConfigDataBatch for display */
    @Override
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        viewModel.getSelectedEntity().observe(this, entity -> {
            configDataBatchEntity = entity;
            binding.setConfigDataBatch(entity);
            setupObjectHeader();
        });
    }
    ...
    public void onNavigationClickedToUsers_UserDetails(View v) {
        ...
        intent.putExtra("parent", configDataBatchEntity);
    ...
    private void onDeleteComplete(@NonNull OperationResult<ConfigDataBatch> result) {
        ...
        listener.onFragmentStateChange(UIConstants.EVENT_DELETION_COMPLETED, configDataBatchEntity);
    }
     ...
     */
    private void setDetailImage(@NonNull ObjectHeader objectHeader, @NonNull ConfigDataBatch configDataBatchEntity) {
        viewModel.downloadMedia(configDataBatchEntity, media -> {
            ...
            if (configDataBatchEntity.getDataValue(ConfigDataBatch.id) != null && !configDataBatchEntity.getDataValue(ConfigDataBatch.id).toString().isEmpty()) {
                objectHeader.setDetailImageCharacter(configDataBatchEntity.getDataValue(ConfigDataBatch.id).toString().substring(0, 1));
            } else {
    ...
    /**
     * Setup ObjectHeader with an instance of ConfigDataBatch
     */
    private void setupObjectHeader() {
        Toolbar secondToolbar = currentActivity.findViewById(R.id.secondaryToolbar);
        if (secondToolbar != null) {
            secondToolbar.setTitle(configDataBatchEntity.getEntityType().getLocalName());
        } else {
            currentActivity.setTitle(configDataBatchEntity.getEntityType().getLocalName());
        }
        ...
        if (objectHeader != null) {
            ...
            objectHeader.setSubheadline(EntityKeyUtil.getOptionalEntityKey(configDataBatchEntity));
            ...
            setDetailImage(objectHeader, configDataBatchEntity);
    ...
    private View setupDataBinding(LayoutInflater inflater, ViewGroup container) {
        binding = FragmentConfigdatabatchsDetailBinding.inflate(inflater, container, false);
