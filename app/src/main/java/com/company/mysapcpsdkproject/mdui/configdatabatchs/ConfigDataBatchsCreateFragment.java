...
import com.company.mysapcpsdkproject.databinding.FragmentConfigdatabatchsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing ConfigDataBatch entity.
 * This fragment is contained in the {@link ConfigDataBatchsActivity}.
 */
public class ConfigDataBatchsCreateFragment extends InterfacedFragment<ConfigDataBatch> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ConfigDataBatchsCreateFragment.class);
    ...
    /** ConfigDataBatch object and it's copy: the modifications are done on the copied object. */
    private ConfigDataBatch configDataBatchEntity;
    private ConfigDataBatch configDataBatchEntityCopy;

    /** DataBinding generated class */
    private FragmentConfigdatabatchsCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** ConfigDataBatch ViewModel */
    ...
    /**
     * This fragment is used for both update and create for ConfigDataBatchs to enter values for the properties.
     ...
     *            ConfigDataBatch if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.configDataBatch.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.configDataBatch.getLocalName();
        }

        ((ConfigDataBatchsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            configDataBatchEntity = createConfigDataBatch();
        } else {
            configDataBatchEntity = viewModel.getSelectedEntity().getValue();
        }

        ConfigDataBatch workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (ConfigDataBatch)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            configDataBatchEntityCopy = (ConfigDataBatch) configDataBatchEntity.copy();
            configDataBatchEntityCopy.setEntityTag(configDataBatchEntity.getEntityTag());
            configDataBatchEntityCopy.setOldEntity(configDataBatchEntity);
            configDataBatchEntityCopy.setEditLink((configDataBatchEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            configDataBatchEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, configDataBatchEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isConfigDataBatchValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((ConfigDataBatchsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(configDataBatchEntityCopy);
        } else {
            viewModel.update(configDataBatchEntityCopy);
        }
        return true;
    }

    /**
     * Create a new ConfigDataBatch instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new ConfigDataBatch instance
     */
    private ConfigDataBatch createConfigDataBatch() {
        ConfigDataBatch configDataBatchEntity = new ConfigDataBatch(true);
        configDataBatchEntity.unsetDataValue(ConfigDataBatch.id);
        return configDataBatchEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<ConfigDataBatch> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(configDataBatchEntityCopy);
            }
            if( isMasterDetail ) {
                ConfigDataBatchsListFragment listFragment = (ConfigDataBatchsListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentConfigdatabatchsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setConfigDataBatch(configDataBatchEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isConfigDataBatchValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_configdatabatch);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.configDataBatch.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<ConfigDataBatch> result) {
