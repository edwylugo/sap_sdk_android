...
import com.company.mysapcpsdkproject.databinding.FragmentParameterssCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing Parameters entity.
 * This fragment is contained in the {@link ParameterssActivity}.
 */
public class ParameterssCreateFragment extends InterfacedFragment<Parameters> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ParameterssCreateFragment.class);
    ...
    /** Parameters object and it's copy: the modifications are done on the copied object. */
    private Parameters parametersEntity;
    private Parameters parametersEntityCopy;

    /** DataBinding generated class */
    private FragmentParameterssCreateBinding binding;
    ...
    /**
     * This fragment is used for both update and create for Parameterss to enter values for the properties.
     ...
     *            Parameters if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.parameters.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.parameters.getLocalName();
        }

        ((ParameterssActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            parametersEntity = createParameters();
        } else {
            parametersEntity = viewModel.getSelectedEntity().getValue();
        }

        Parameters workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (Parameters)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            parametersEntityCopy = (Parameters) parametersEntity.copy();
            parametersEntityCopy.setEntityTag(parametersEntity.getEntityTag());
            parametersEntityCopy.setOldEntity(parametersEntity);
            parametersEntityCopy.setEditLink((parametersEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            parametersEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, parametersEntityCopy);
    ...
    private boolean onSaveItem() {
        ...
        ((ParameterssActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(parametersEntityCopy);
        } else {
            viewModel.update(parametersEntityCopy);
        }
        return true;
    }

    /**
     * Create a new Parameters instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new Parameters instance
     */
    private Parameters createParameters() {
        Parameters parametersEntity = new Parameters(true);
        parametersEntity.unsetDataValue(Parameters.id);
        return parametersEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<Parameters> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(parametersEntityCopy);
            }
            if( isMasterDetail ) {
                ParameterssListFragment listFragment = (ParameterssListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentParameterssCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setParameters(parametersEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isParametersValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_parameters);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.parameters.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<Parameters> result) {
