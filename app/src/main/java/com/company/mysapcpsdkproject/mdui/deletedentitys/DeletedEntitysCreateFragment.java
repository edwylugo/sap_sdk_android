...
import com.company.mysapcpsdkproject.databinding.FragmentDeletedentitysCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing DeletedEntity entity.
 * This fragment is contained in the {@link DeletedEntitysActivity}.
 */
public class DeletedEntitysCreateFragment extends InterfacedFragment<DeletedEntity> {

    private static final Logger LOGGER = LoggerFactory.getLogger(DeletedEntitysCreateFragment.class);
    ...
    /** DeletedEntity object and it's copy: the modifications are done on the copied object. */
    private DeletedEntity deletedEntityEntity;
    private DeletedEntity deletedEntityEntityCopy;

    /** DataBinding generated class */
    private FragmentDeletedentitysCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** DeletedEntity ViewModel */
    ...
    /**
     * This fragment is used for both update and create for DeletedEntitys to enter values for the properties.
     ...
     *            DeletedEntity if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.deletedEntity.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.deletedEntity.getLocalName();
        }

        ((DeletedEntitysActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            deletedEntityEntity = createDeletedEntity();
        } else {
            deletedEntityEntity = viewModel.getSelectedEntity().getValue();
        }

        DeletedEntity workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (DeletedEntity)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            deletedEntityEntityCopy = (DeletedEntity) deletedEntityEntity.copy();
            deletedEntityEntityCopy.setEntityTag(deletedEntityEntity.getEntityTag());
            deletedEntityEntityCopy.setOldEntity(deletedEntityEntity);
            deletedEntityEntityCopy.setEditLink((deletedEntityEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            deletedEntityEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, deletedEntityEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isDeletedEntityValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((DeletedEntitysActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(deletedEntityEntityCopy);
        } else {
            viewModel.update(deletedEntityEntityCopy);
        }
        return true;
    }

    /**
     * Create a new DeletedEntity instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new DeletedEntity instance
     */
    private DeletedEntity createDeletedEntity() {
        DeletedEntity deletedEntityEntity = new DeletedEntity(true);
        deletedEntityEntity.unsetDataValue(DeletedEntity.id);
        return deletedEntityEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<DeletedEntity> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(deletedEntityEntityCopy);
            }
            if( isMasterDetail ) {
                DeletedEntitysListFragment listFragment = (DeletedEntitysListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentDeletedentitysCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setDeletedEntity(deletedEntityEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isDeletedEntityValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_deletedentity);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.deletedEntity.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<DeletedEntity> result) {
