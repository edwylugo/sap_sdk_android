...
import com.company.mysapcpsdkproject.databinding.FragmentLongtextorderbatchsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing LongTextOrderBatch entity.
 * This fragment is contained in the {@link LongTextOrderBatchsActivity}.
 */
public class LongTextOrderBatchsCreateFragment extends InterfacedFragment<LongTextOrderBatch> {

    private static final Logger LOGGER = LoggerFactory.getLogger(LongTextOrderBatchsCreateFragment.class);
    ...
    /** LongTextOrderBatch object and it's copy: the modifications are done on the copied object. */
    private LongTextOrderBatch longTextOrderBatchEntity;
    private LongTextOrderBatch longTextOrderBatchEntityCopy;

    /** DataBinding generated class */
    private FragmentLongtextorderbatchsCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** LongTextOrderBatch ViewModel */
    ...
    /**
     * This fragment is used for both update and create for LongTextOrderBatchs to enter values for the properties.
     ...
     *            LongTextOrderBatch if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.longTextOrderBatch.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.longTextOrderBatch.getLocalName();
        }

        ((LongTextOrderBatchsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            longTextOrderBatchEntity = createLongTextOrderBatch();
        } else {
            longTextOrderBatchEntity = viewModel.getSelectedEntity().getValue();
        }

        LongTextOrderBatch workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (LongTextOrderBatch)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            longTextOrderBatchEntityCopy = (LongTextOrderBatch) longTextOrderBatchEntity.copy();
            longTextOrderBatchEntityCopy.setEntityTag(longTextOrderBatchEntity.getEntityTag());
            longTextOrderBatchEntityCopy.setOldEntity(longTextOrderBatchEntity);
            longTextOrderBatchEntityCopy.setEditLink((longTextOrderBatchEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            longTextOrderBatchEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, longTextOrderBatchEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isLongTextOrderBatchValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((LongTextOrderBatchsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(longTextOrderBatchEntityCopy);
        } else {
            viewModel.update(longTextOrderBatchEntityCopy);
        }
        return true;
    }

    /**
     * Create a new LongTextOrderBatch instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new LongTextOrderBatch instance
     */
    private LongTextOrderBatch createLongTextOrderBatch() {
        LongTextOrderBatch longTextOrderBatchEntity = new LongTextOrderBatch(true);
        longTextOrderBatchEntity.unsetDataValue(LongTextOrderBatch.id);
        return longTextOrderBatchEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<LongTextOrderBatch> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(longTextOrderBatchEntityCopy);
            }
            if( isMasterDetail ) {
                LongTextOrderBatchsListFragment listFragment = (LongTextOrderBatchsListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentLongtextorderbatchsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setLongTextOrderBatch(longTextOrderBatchEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isLongTextOrderBatchValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_longtextorderbatch);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.longTextOrderBatch.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<LongTextOrderBatch> result) {
