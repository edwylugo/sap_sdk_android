...
import com.company.mysapcpsdkproject.databinding.FragmentOrderruletypesCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing OrderRuleType entity.
 * This fragment is contained in the {@link OrderRuleTypesActivity}.
 */
public class OrderRuleTypesCreateFragment extends InterfacedFragment<OrderRuleType> {

    private static final Logger LOGGER = LoggerFactory.getLogger(OrderRuleTypesCreateFragment.class);
    ...
    /** OrderRuleType object and it's copy: the modifications are done on the copied object. */
    private OrderRuleType orderRuleTypeEntity;
    private OrderRuleType orderRuleTypeEntityCopy;

    /** DataBinding generated class */
    private FragmentOrderruletypesCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** OrderRuleType ViewModel */
    ...
    /**
     * This fragment is used for both update and create for OrderRuleTypes to enter values for the properties.
     ...
     *            OrderRuleType if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.orderRuleType.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.orderRuleType.getLocalName();
        }

        ((OrderRuleTypesActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            orderRuleTypeEntity = createOrderRuleType();
        } else {
            orderRuleTypeEntity = viewModel.getSelectedEntity().getValue();
        }

        OrderRuleType workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (OrderRuleType)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            orderRuleTypeEntityCopy = (OrderRuleType) orderRuleTypeEntity.copy();
            orderRuleTypeEntityCopy.setEntityTag(orderRuleTypeEntity.getEntityTag());
            orderRuleTypeEntityCopy.setOldEntity(orderRuleTypeEntity);
            orderRuleTypeEntityCopy.setEditLink((orderRuleTypeEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            orderRuleTypeEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, orderRuleTypeEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isOrderRuleTypeValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((OrderRuleTypesActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(orderRuleTypeEntityCopy);
        } else {
            viewModel.update(orderRuleTypeEntityCopy);
        }
        return true;
    }

    /**
     * Create a new OrderRuleType instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new OrderRuleType instance
     */
    private OrderRuleType createOrderRuleType() {
        OrderRuleType orderRuleTypeEntity = new OrderRuleType(true);
        orderRuleTypeEntity.unsetDataValue(OrderRuleType.orderType);
        return orderRuleTypeEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<OrderRuleType> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(orderRuleTypeEntityCopy);
            }
            if( isMasterDetail ) {
                OrderRuleTypesListFragment listFragment = (OrderRuleTypesListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentOrderruletypesCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setOrderRuleType(orderRuleTypeEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isOrderRuleTypeValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_orderruletype);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.orderRuleType.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<OrderRuleType> result) {
